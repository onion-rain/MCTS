...
def Binary_conv3x3(in_channels, out_channels, stride=1, groups=1, padding=1):
    ...
    return BinaryConv2d(in_channels, out_channels, kernel_size=3, stride=stride,
                     padding=padding, groups=groups, bias=False, dilation=1)

def Binary_conv1x1(in_channels, out_channels, stride=1):
    """1x1 convolution"""
    return BinaryConv2d(in_channels, out_channels, kernel_size=1, stride=stride, bias=False)
...
class Basicneck(nn.Module):
    ...
    def __init__(self, in_channels, mid_channels, out_channels, stride=1):
        ...
        self.norm1 = nn.BatchNorm2d(in_channels) # norm前置保证activation mean为0
        self.conv1 = Binary_conv3x3(in_channels, mid_channels, stride=stride)
        # self.norm1 = nn.BatchNorm2d(mid_channels)

        self.norm2 = nn.BatchNorm2d(mid_channels) # norm前置保证activation mean为0
        self.conv2 = Binary_conv3x3(mid_channels, out_channels, stride=1)
        # self.norm2 = nn.BatchNorm2d(out_channels)

        if stride != 1 or in_channels != out_channels:
            self.conv_shortcut = Binary_conv1x1(in_channels, out_channels, stride=stride)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels, mid_channels, out_channels, stride=1):
        ...
        self.norm1 = nn.BatchNorm2d(in_channels) # norm前置保证activation mean为0
        self.conv1 = Binary_conv1x1(in_channels, mid_channels, stride=1)
        # self.norm1 = nn.BatchNorm2d(mid_channels)

        self.norm2 = nn.BatchNorm2d(mid_channels) # norm前置保证activation mean为0
        self.conv2 = Binary_conv3x3(mid_channels, mid_channels, stride=stride)
        # self.norm2 = nn.BatchNorm2d(mid_channels)

        self.norm3 = nn.BatchNorm2d(mid_channels) # norm前置保证activation mean为0
        self.conv3 = Binary_conv1x1(mid_channels, out_channels, stride=1)
...
def resnet18_xnornet(num_classes=10):
    return ResNet(Basicneck, [2, 2, 2, 2], num_classes)

def resnet34_xnornet(num_classes=10):
    return ResNet(Basicneck, [3, 4, 6, 3], num_classes)

def resnet50_xnornet(num_classes=10):
    return ResNet(Bottleneck, [3, 4, 6, 3], num_classes)

def resnet101_xnornet(num_classes=10):
    return ResNet(Bottleneck, [3, 4, 23, 3], num_classes)

def resnet152_xnornet(num_classes=10):
