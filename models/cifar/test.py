...
def conv3x3(in_planes, out_planes, wbit, abit, stride=1):
    ...
    return QuantizedConv2d(a_bits=abit, w_bits=wbit, g_bits=32,
...
def conv1x1(in_planes, out_planes, wbit, abit, stride=1):
    """1x1 convolution"""
    return QuantizedConv2d(a_bits=abit, w_bits=wbit, g_bits=32,
                in_channels=in_planes, out_channels=out_planes, kernel_size=1, stride=stride, 
                bias=False, dilation=1)

def linear(in_featrues, out_features, wbit, abit):
    return nn.Linear(in_featrues, out_features)
...
def test(type='dorefa', a_bits=4, w_bits=4, g_bits=32, num_classes=10):
    return ResNet(BasicBlock, [2,2,2,2], wbit=w_bits, abit=a_bits)